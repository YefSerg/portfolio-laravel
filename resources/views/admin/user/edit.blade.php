...
@section('content')
    <x-admin.title-content title="Edit user"/>
    <form method="POST" action="{{ route('admin.user.update', compact('user')) }}">
        @csrf
        @method('PATCH')
                    ...
                    <select name="role" class="form-control select2 user__status-select2" style="width: 100%;">
                        @foreach($roles as $role)
                            <option value="{{ $role->value }}"
                                    @if($role->value === $user->role->value) selected="selected" @endif>
                                {{ $role->getLabel() }}
                            </option>
                        @endforeach
                    </select>
